...
static __INLINE uint8_t wireless_reg_read(uint8_t reg)
{
	uint8_t rx_data[2];
	uint8_t tx_data[2];
	tx_data[0] = (reg & 0x1F)| NRF24L01_CMD_R_REGISTER;
	tx_data[1] = 0;
	wireless_CSN_low();
...
static __INLINE void wireless_reg_write(uint8_t reg, uint8_t data)
{
	...
	uint8_t rx_data[2];
	uint8_t tx_data[2];
	tx_data[0] = ((reg + (0x01 << 5)) & 0x3F) | NRF24L01_CMD_R_REGISTER;
	tx_data[1] = data;
	wireless_CSN_low();
	spiTx(RF_SPI_BASE, tx_data, 2, rx_data);
	wireless_CSN_high();
...
{
	uint8_t * rx_data;
	wireless_reg_write(((NRF24L01_TX_ADDR_R+ (0x01 << 5)) & 0x3F) , *tx_addr);
	wireless_CSN_low();
	spiTx(RF_SPI_BASE, (tx_addr + 1), 5, rx_data);
	wireless_CSN_high();
}
...
static __INLINE void wireless_tx_data_payload( uint32_t data)
{
	...
	uint8_t tx_addr = NRF24L01_CMD_W_TX_PAYLOAD;
	wireless_CSN_low();
	spiTx(RF_SPI_BASE , &tx_addr, 1, rx_data);
	spiTx(RF_SPI_BASE , &data3, 1, rx_data);
	spiTx(RF_SPI_BASE , &data2, 1, rx_data);
	spiTx(RF_SPI_BASE , &data1, 1, rx_data);
	spiTx(RF_SPI_BASE , &data0, 1, rx_data);
	wireless_CSN_high();
...
static __INLINE void wireless_rx_data_payload( uint32_t *data)
{
	...
	uint8_t data1;
	uint8_t data0;
	uint8_t tx_addr =NRF24L01_CMD_R_RX_PAYLOAD;
	wireless_CSN_low();
	spiTx(RF_SPI_BASE , &tx_addr, 1, &data3);
	spiTx(RF_SPI_BASE , tx_data, 1, &data3);
	spiTx(RF_SPI_BASE , tx_data, 1, &data2);
	spiTx(RF_SPI_BASE , tx_data, 1, &data1);
	spiTx(RF_SPI_BASE , tx_data, 1, &data0);
	wireless_CSN_high();
